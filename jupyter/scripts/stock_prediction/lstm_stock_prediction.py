...
# Build LSTM model with Dropout for better generalization
model = Sequential()
...
print("\nModel saved to models/lstm_tsla_model.h5")
...
plt.title("TSLA Stock Price: LSTM Model")
plt.xlabel("Date")
...
plt.title(f"LSTM Test Set Predictions\nRMSE: ${rmse:.2f}, MAE: ${mae:.2f}, MAPE: {mape:.2f}%")
plt.xlabel("Date")
plt.ylabel("Price (USD)")
plt.legend()
plt.grid(True, alpha=0.3)
...
plt.tight_layout()
plt.savefig('models/lstm_prediction.png', dpi=100, bbox_inches='tight')
print("Plot saved to models/lstm_prediction.png")
plt.show()
